...
void MINDplotter::define_tree_branches() {
  ...
  statTree->Branch("trajsAngle", &_trajsAngle,32000,0);
...
void MINDplotter::patternStats2(fitter& Fit) {
  ...
  if(traj.size()==1)
    {
      //find angle between hits. (In TASD preferably)
      //Just take hits 1,2 for the different tracks.

      const Measurement& meas11 = traj[0]->node(0).measurement();
      const Measurement& meas12 = traj[0]->node(1).measurement();
      //const Measurement& meas11 = traj[0]->node(traj[0]->size()-1).measurement();
      //const Measurement& meas12 = traj[0]->node(traj[0]->size()-2).measurement();
      //const Measurement& meas21 = traj[1]->node(0).measurement();
      //const Measurement& meas22 = traj[1]->node(1).measurement();
      cout<<"1"
	  <<"\t"<<meas11.position()[0]
	  <<"\t"<<meas11.position()[1]
	  <<"\t"<<meas11.position()[2]
      ...
      double x1 = meas12.position()[0]-meas11.position()[0];
      double y1 = meas12.position()[1]-meas11.position()[1];
      double z1 = meas12.position()[2]-meas11.position()[2];

      cout<<"diff"<<x1<<"\t"<<y1<<"\t"<<z1<<endl;

      // Angle between tracks
      //double x2 = meas22.position()[0]-meas21.position()[0];
      //double y2 = meas22.position()[1]-meas21.position()[1];
      //double z2 = meas22.position()[2]-meas21.position()[2];
      ...
      double cosTheta = (x1*x2+y1*y2+z1*z2)/(sqrt(x1*x1+y1*y1+z1*z1)*sqrt(x2*x2+y2*y2+z2*z2));

      double theta = acos(cosTheta);

      _trajsAngle.push_back(theta);

      cout<<"cosTheta="<<cosTheta<<endl;
      cout<<"theta="<<theta<<endl;

      cout<<"AngleCheck"<<endl;
      cout<<acos(0)<<endl;
      cout<<acos(1/sqrt(2))<<endl;
      cout<<acos(-1/sqrt(2))<<endl;
      cout<<acos(1)<<endl;
      cout<<acos(-1)<<endl;
