...
bool event_classif::Execute(const vector<cluster*>& hits,
  ...
  */
  ...
  while (hits2.size() >= 4 && (int)vtrajs.size() <= _maxNtraj) { 
    //cout<<"hits2.size()="<<hits2.size()<<endl;
    ///create trajectory     
...
void event_classif::readParam() {
 ...
 if ( _infoStore.find_istore("likeli") )
  ...
  _tolerance = _infoStore.fetch_dstore("pos_resZ") * cm;
...
if not found then  excluded_hits = 0; _exclPlanes = 0; i.e, vertGuess =0*/
  ...
  //for(int pl=_planes.size()-2; pl >= 1; pl-- ){
    ...
    //if(_planes[pl]->GetNHits()!=1) break;
    if(_planes[pl]->GetNHits()!=1) continue;
...
bool event_classif::get_patternRec_seed(State& seed, Trajectory& muontraj){
  ...
  V[3] = (muontraj.nodes()[0]->measurement().vector()[0] -muontraj.nodes()[1]->measurement().vector()[0])/
    (muontraj.nodes()[0]->measurement().vector()[2] -muontraj.nodes()[1]->measurement().vector()[2]);
  V[4] = (muontraj.nodes()[0]->measurement().vector()[1] -muontraj.nodes()[1]->measurement().vector()[1])/
    (muontraj.nodes()[0]->measurement().vector()[2] -muontraj.nodes()[1]->measurement().vector()[2]);
...
//***********************************************************************
bool event_classif::perform_muon_extraction2(const State& seed, vector<cluster*>& hits,
					    Trajectory& muontraj, vector<cluster*>& hads) {
  //***********************************************************************
  //Loop through multiple occupancy planes finding the best match to the muon
  //traj and filtering it into the trajectory.
  _m.message("I am  event_classif::perform_muon_extraction2 ", bhep::DETAILED); 
  bool ok;
  long ChiMin;
  int nConsecHole = 0;
  //cout<<"size0:"<<muontraj.size()<<endl;
...
bool event_classif::LowMomentumExtraction(vector<cluster*>& hits,
  ...
  V[5] = 1/p;
  //M[5][5]=1.0/((0.1*p)*(0.1*p));
  //M[5][5]= p*p/(100*100*100*100*100*10*10);
  //M[5][5]=1/p*1/p*1/p*1/p;
  M[5][5]=0.1/p * 0.1/p;
